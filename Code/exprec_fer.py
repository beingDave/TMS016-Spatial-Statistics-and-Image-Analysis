...
# model file names
model_names = ['model_fer2013_high_val_acc.pt', 'model_fer2013_low_val_loss.pt']
...
def load_train_data():
    fer_data = pd.read_csv(data_path+'\\fer2013training.csv')
...
def load_test_data():
    fer_data1 = pd.read_csv(data_path+'\\fer2013training.csv')
...
# ln_rate = 0.1
# ln_factor = 0.1
# optimizer = optim.SGD(Model.parameters(), momentum=0.2, nesterov=True, lr=ln_rate)
# scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=ln_factor, patience=1, verbose=True)
...
early_stopping = EarlyStopping(patience=20, verbose=True, delta=ln_rate*0.5)
...
for epoch in range(1,n_epochs + 1):
    ...
    for data,target in train_pictures:
        if len(data) > 0:
            optimizer.zero_grad()
            data = data[:][:,6:-6,6:-6]
            data = torch.FloatTensor(data).view(-1,1,36,36)
...
for ii in range(2):
    ...
    cm_file = "ConfusionMatrix_"+model_names[ii]+"_.png"
...
ax1.imshow(X_train[z,:,:], cmap='gray')
ax1.grid(None)
ax1.set_title(CATEGORIES[y_train[z]])
ax2 = fig.add_subplot(2,3,2)
ax2.grid(None)
ax2.imshow(X_train[z+50,:,:], cmap='gray')
ax2.set_title(CATEGORIES[y_train[z+50]])
ax3 = fig.add_subplot(2,3,3)
ax3.grid(None)
ax3.imshow(X_train[z+77,:,:], cmap='gray')
ax3.set_title(CATEGORIES[y_train[z+77]])
ax4 = fig.add_subplot(2,3,4)
ax4.grid(None)
ax4.imshow(X_train[z+100,:,:], cmap='gray')
ax4.set_title(CATEGORIES[y_train[z+100]])
ax5 = fig.add_subplot(2,3,5)
ax5.imshow(X_train[z+150,:,:], cmap='gray')
ax5.grid(None)
ax5.set_title(CATEGORIES[y_train[z+150]])
ax6 = fig.add_subplot(2,3,6)
ax6.imshow(X_train[z+203,:,:], cmap='gray')
ax6.grid(None)
ax6.set_title(CATEGORIES[y_train[z+203]])
fig.savefig("SampleData.png")
